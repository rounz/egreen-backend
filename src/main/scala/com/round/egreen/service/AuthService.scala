...
class UserAuth(config: Config) {
  ...
  def apply[F[_]: Effect](service: AuthedService[User, F]): HttpService[F] =
    AuthMiddleware(authUser).apply(service)

  def admin[F[_]: Effect](service: AuthedService[User, F]): HttpService[F] =
  ...
  private def authUser[F[_]: Effect]: Kleisli[OptionT[F, ?], Request[F], User] =
    Kleisli { request =>
      OptionT.fromOption[F] {
        for {
          ...
          UserClaim(user, expr, _)            <- json.as[UserClaim].toOption if Instant.now.getEpochSecond <= expr
        } yield user
      }
    }

  private def authAdmin[F[_]: Effect]: Kleisli[OptionT[F, ?], Request[F], User] =
    authRole(Admin)

  private def authRole[F[_]: Effect](role: Role): Kleisli[OptionT[F, ?], Request[F], User] =
    authUser.andThen { u =>
      OptionT.fromOption(
...
object UserAuth {
  ...
  final private case class UserClaim(user: User, expiration: Long, issuedAt: Long)
